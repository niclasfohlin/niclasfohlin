...
6. När kön är genomarbetad, och först då: `git switch main && git merge natt/<datum> && git push`. En enda push för hela natten, eftersom varje bygge på Netlify drar krediter. Vänta in bygget och kontrollera att det blev grönt; blev det rött, laga på en ny commit eller backa med `git revert`.
